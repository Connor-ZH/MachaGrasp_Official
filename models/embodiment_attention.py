"""
Embodiment-specific transformer layers adapted from GET-Zero.
Modified MultiHeadAttention internals are adapted from PyTorch to support
custom weights in attention computation.
"""
...
from models.generic import pad_stack_tensors
...
import numpy as np
from morphology.utils.embodiment_util import EmbodimentProperty, compute_spd_matrix
...
class SpatialEncodingTransformerEncoderLayer(TransformerEncoderLayer):
    def __init__(self, embodiment_properties_by_id: List[EmbodimentProperty], max_dof_count: int, attention_config: Dict, d_model: int, nhead: int, dim_feedforward: int = 2048, dropout: float = 0.1,
        ...
        # Adjacency matrix
        adjacency_matrices_by_id = pad_stack_tensors([ep.adjacency_matrix for ep in embodiment_properties_by_id], pad_value=0) # (B, max_provided_dof_count, max_provided_dof_count)

        if adjacency_matrices_by_id is not None:
            # Shortest path distance (SPD) matrices
            if self.enable_spatial_encoding:
                spd_matrices_by_id = pad_stack_tensors([ep.get_spatial_distance_matrix(refine=False) for ep in embodiment_properties_by_id])
            ...
            if self.enable_parent_encoding:
                parent_matrices_by_id = pad_stack_tensors([ep.get_parent_distance_matrix(refine=False) for ep in embodiment_properties_by_id], pad_value=0)
                self.register_buffer('parent_matrices_by_id', parent_matrices_by_id, persistent=False)

            if self.enable_child_encoding:
                child_matrices_by_id = pad_stack_tensors([ep.get_child_distance_matrix(refine=False) for ep in embodiment_properties_by_id], pad_value=0)
                self.register_buffer('child_matrices_by_id', child_matrices_by_id, persistent=False)
        ...
        max_node_to_node_distance = max_dof_count - 1
        if self.enable_spatial_encoding:
            self.spatial_embedding = nn.Embedding(max_node_to_node_distance + 2, nhead, padding_idx=0)
        if self.enable_parent_encoding:
            self.parent_embedding = nn.Embedding(max_node_to_node_distance + 1, nhead, padding_idx=0)
        if self.enable_child_encoding:
            self.child_embedding = nn.Embedding(max_node_to_node_distance + 1, nhead, padding_idx=0)
    ...
    def _sa_block(self, embodiment_ids: Tensor, x: Tensor,
        ...
        if self.enable_spatial_encoding:
            ...
            spatial_bias = self.spatial_embedding(spd_matrices)# (b, S, S, n_head)

            if torch.isnan(spd_matrices).any():
                print("gradient: ", self.spatial_embedding.weight.grad)
                print("weight: ", self.spatial_embedding.weight)
                raise "NaN detected in spd_matrices!"

            if torch.isnan(spatial_bias).any():
                print("gradient: ",self.spatial_embedding.weight.grad)
                print("weight: ",self.spatial_embedding.weight)
                raise "NaN detected in spatial_embedding!"

            if not torch.all(spatial_bias.permute(0, 2, 1, 3) == spatial_bias):
                print("spd_matrices: ", spd_matrices)
                print("spatial_bias: ", spatial_bias)
                print("spatial_bias permute: ", spatial_bias.permute(0, 2, 1, 3))
                raise 'spatial bias should be symmetric'
        ...
        if self.enable_parent_encoding:
            ...
            if torch.isnan(parent_bias).any():
                print("gradient: ",self.parent_embedding.weight.grad)
                print("weight: ",self.parent_embedding.weight)
                raise "NaN detected in parent embeddings!"
        ...
        if self.enable_child_encoding:
            ...
            if torch.isnan(child_bias).any():
                print("gradient: ",self.child_embedding.weight.grad)
                print("weight: ",self.child_embedding.weight)
                raise "NaN detected in child_embedding!"
...
def custom_attention_weights_multi_head_attention_forward(
    ...
    tgt_len: The number of tokens in the sequence (or target length).
    bsz: The batch size (number of sequences in the batch).
    embed_dim: The final embedding dimension for each token after multi-head attention and the linear projection.
    '''
